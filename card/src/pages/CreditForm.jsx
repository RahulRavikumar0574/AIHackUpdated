...
const CreditForm = () => {
  ...
  const validate = () => {
    const e = {};
    const isPosInt = (v) => Number.isFinite(v) && v >= 0;
    const ageNum = Number(formData.age);
    if (!Number.isFinite(ageNum) || ageNum < 18 || ageNum > 100) {
      e.age = 'Age must be between 18 and 100';
    }
    const fam = Number(formData.familyMembers);
    if (!isPosInt(fam) || fam < 1 || fam > 20) {
      e.familyMembers = 'Family members must be between 1 and 20';
    }
    const inc = Number(formData.income);
    if (!isPosInt(inc)) {
      e.income = 'Income must be a non-negative number';
    }
    if (formData.employmentStatus === 'Employed') {
      const yrs = Number(formData.yearsEmployed);
      if (!isPosInt(yrs) || yrs > 80) {
        e.yearsEmployed = 'Years employed must be between 0 and 80';
      }
    }
    const children = Number(formData.children);
    if (!isPosInt(children) || children > 20) {
      e.children = 'Children must be between 0 and 20';
    }
    return e;
  };
      ...
      <div className="relative z-10 w-full p-8 text-white">
        ...
        {prediction && (
          ...
          >
            ...
            {(prediction.risk_probability_default !== undefined || prediction.decision_threshold !== undefined) && (
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-800">
                {prediction.risk_probability_default !== undefined && (
                  <div className="bg-white bg-opacity-70 p-3 rounded">
                    <div className="font-semibold text-gray-700">Estimated Default Risk</div>
                    <div className="text-gray-900 text-lg">
                      {`${(prediction.risk_probability_default * 100).toFixed(1)}%`}
                    </div>
                  </div>
                )}
                {prediction.decision_threshold !== undefined && (
                  <div className="bg-white bg-opacity-70 p-3 rounded">
                    <div className="font-semibold text-gray-700">Decision Threshold</div>
                    <div className="text-gray-900 text-lg">
                      {`${(prediction.decision_threshold * 100).toFixed(1)}%`}
                    </div>
                  </div>
                )}
              </div>
            ...
            {prediction.risk_probability_default !== undefined && prediction.decision_threshold !== undefined && (
              <div className="mt-6 flex justify-center">
                <RiskGauge
                  risk={prediction.risk_probability_default}
            ...
            {/* Debug block */}
            {showDebug && prediction._debug && (
              <div className="mt-2 p-3 bg-white bg-opacity-80 rounded text-gray-800 text-sm overflow-x-auto">
                <pre className="whitespace-pre-wrap break-words">{JSON.stringify(prediction._debug, null, 2)}</pre>
              </div>
            )}
          </div>
        )}
          ...
          <div className="mt-8 p-6 rounded-lg bg-white bg-opacity-80 text-gray-800 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="flex flex-col items-center">
                ...
                <img
                  src={`${import.meta.env.VITE_API_BASE || 'http://localhost:8000'}/static/credit_analysis_model.png`}
                  alt="Model Performance"
                  className="w-full h-auto rounded-lg shadow-md cursor-pointer"
                  onClick={() => handleImageClick(`${import.meta.env.VITE_API_BASE || 'http://localhost:8000'}/static/credit_analysis_model.png`)}
                />
              </div>
              <div className="flex flex-col items-center">
                <h3 className="text-xl font-semibold mb-2">Demographic Insights</h3>
                <img
                  src={`${import.meta.env.VITE_API_BASE || 'http://localhost:8000'}/static/credit_analysis_demographics.png`}
                  alt="Demographic Insights"
                  className="w-full h-auto rounded-lg shadow-md cursor-pointer"
                  onClick={() => handleImageClick(`${import.meta.env.VITE_API_BASE || 'http://localhost:8000'}/static/credit_analysis_demographics.png`)}
                />
              </div>
            </div>
          </div>
