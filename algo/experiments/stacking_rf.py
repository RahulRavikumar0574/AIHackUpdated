...
def run_stacking_rf(df: pd.DataFrame, target_col='RISK_ASSESSMENT', random_state=42):
    print("=== Stacking Ensemble with RandomForest Meta-Learner ===")
    ...
    # Train meta-learner (RandomForest)
    print("Training RandomForest meta-learner...")
    meta_model = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=random_state)
    ...
    print(f"\nStacking RF Meta -> AP: {ap:.4f}, ROC AUC: {roc:.4f}, Youden thr: {best_threshold:.3f}")
...
if __name__ == '__main__':
    ...
    run_stacking_rf(df, target_col=target_col)
