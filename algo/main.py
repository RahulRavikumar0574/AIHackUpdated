...
# Allow overriding CORS origins via env var ALLOWED_ORIGINS (comma-separated)
...
def health():
    ...
    ok = bool(model is not None and model_columns is not None)
    return {
        "status": "ok" if ok else "degraded",
        "model_loaded": ok,
        "ensemble_available": bool('ensemble_enabled' in globals() and ensemble_enabled),
    }
...
@app.get("/artifacts")
def get_artifacts():
    """Expose current artifacts and ensemble configuration for observability."""
    artifacts = {}
    try:
        artifacts_path = os.path.join(os.path.dirname(__file__), 'artifacts.json')
        if os.path.exists(artifacts_path):
    ...
    try:
        ens_info = {
            "enabled": bool('ensemble_enabled' in globals() and ensemble_enabled),
            "models": ens_model_names,
            "feature_count": len(ens_feature_columns) if 'ens_feature_columns' in globals() and ens_feature_columns is not None else None,
        }
    except Exception:
        ens_info = {"enabled": False}
